...
export default class stats {
    ...
    if (!content) {
      ...
      try {
        return message.reply(
          ...
          } does not yet have any stats :( they need to post a message in the server to be registered by me.`
        );
      } catch (err_1) {
        console.log(err_1);
      }
  ...
  getEmbed(client: any, member: GuildMember, content: any) {
    let name =
      "**" +
    ...
    let rankColor = member.guild.roles.find(
      role => role.name === content.rank.name
    )?.color;
    ...
    if (content.rank.xp !== null)
      levelStats += `**xp:**  *${this.getFormattedNumber(
        content.rank.xp
      )} / ${this.getFormattedNumber(content.rank.levelup)}*\n`.toUpperCase();
    ...
    if (member.roles.has(client.config.roles.mod)) {
      embed.setFooter(
        "BE RESPECTFUL TO ALL - ESPECIALLY MODERATORS",
        "https://i.ibb.co/MC5389q/crossed-swords-2694.png"
      );
  ...
  calculatePosition(client: any, content: any) {
    ...
    for (let [username, userContent] of entries)
      if (
        ...
        (userContent as any).rank.xp > content.rank.xp
      )
        usersHigher++;
